...
class LogLikeWithGrad(tt.Op): # ????
    ...
    def __init__(self, loglike, data, engine):
        ...
        self.logpgrad = LogLikeGrad(self.likelihood, self.data, self.engine)

    def perform(self, node, inputs, outputs):
        # the method that is used when calling the Op
        (parameters,) = inputs  # this will contain my variables ???
        # call the log-likelihood function
        logl = self.likelihood(parameters, self.data, self.engine)
...
class LogLikeGrad(tt.Op):
    ...
    def perform(self, node, inputs, outputs):
        ...
        def lnlike(values):
            return self.likelihood(values, self.data, self.engine)
...
def my_loglike(parameters, data, engine, ):
    ...
    loglik = -.5*np.log(np.linalg.det(v_covar)) - (64/2) * np.log(2 * 3.14159) - .5 * (1/np.square(sigma)) * np.sum((data - voltages) ** 2) #log likelihood equation
    #print(loglik)
    return loglik
...
data = mat2['volt2']
covar = spio.loadmat('C:/Users/krait/covarmatrix.mat', squeeze_me=True)
covar2 = covar['covar']
#data = data.tolist()
#sigma = 1.2

# create our Op
logl = LogLikeWithGrad(my_loglike, data, engine)
